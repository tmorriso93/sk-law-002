...
function HomeBanner() {
  ...
  return (
    <div className="overflow-x-auto ">
      {/* Adjusted height */}
      {/* <section className="relative max-w-full bg-cover bg-center bg-no-repeat h-[60vh] md:h-[70vh]">  */}
      <section className="relative max-w-full bg-cover bg-center bg-no-repeat h-[60vh] md:h-full"> 
      <div className="absolute inset-0 z-0 w-full h-full ">
    <Image
      src={slides[currentSlide]}
      alt="Background"
      layout="fill" // Ensures the image fills the parent element
      objectFit="cover" // Maintains aspect ratio and covers the parent fully
      objectPosition="center" // Centers the image
      quality={100} // Ensures high-quality rendering
      priority // Marks it as a priority for loading
      className="w-full h-full bg-contain bg-center" // Ensures full dimensions within the parent
    />
  </div>

        <div className="absolute inset-0 bg-gray-900/75 lg:bg-gray-900/75 md:bg-gray-900/75 sm:bg-gray-900/75 sm:bg-transparent sm:from-gray-900/95 sm:to-gray-900/25 ltr:sm:bg-gradient-to-r rtl:sm:bg-gradient-to-l"></div>

        <div className="relative mx-auto max-w-[1200px] px-4 lg:px-8 flex flex-col justify-center items-center h-full"> {/* Flexbox for centering */}
          <div id="content" className="text-center">
            ...
            <div id="copy" className="max-sm:mt-[-50px] mt-[-50px] ">
              {/* <h1 className="drop-shadow-md text-3xl font-extrabold text-white sm:text-5xl font-didot">
                <strong className="block font-extrabold uppercase drop-shadow-lg text-slate-50"> King Law </strong>
              </h1> */}
              ...
              <div className="mt-4 flex flex-wrap gap-4 text-center max-sm:pb-20 pb-8">
                <Link href="https://kinglawpc.cliogrow.com/book" passHref legacyBehavior>
                  <a
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block font-bold w-full m-auto rounded px-12 py-3 text-sm text-white shadow-md
                     hover:shadow-xl hover:scale-105 transform transition-all duration-300 ease-in-out focus:outline-none
                      focus:ring sm:w-auto"
                    style={{ backgroundColor: "#1d2b4f" }}
                  >
                    Book a Consultation
                  </a>
        ...
        {/* Slider controls */}
        <button onClick={prevSlide} className="absolute left-4 top-1/2 transform -translate-y-1/2 text-white max-sm:pb-36">
          &#10094;
        </button>
        <button onClick={nextSlide} className="absolute right-4 top-1/2 transform -translate-y-1/2 text-white max-sm:pb-36">
          &#10095;
        </button>
      </section>
    </div>
